...
export default function ControllerPage() {
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      setSelectedItems({});
      setShowCart(false);
      localStorage.removeItem("selectedItems"); // clear cart after order
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6 flex justify-center">
          <input
            ...
            className="w-full max-w-md px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
        </div>

        {/* Items List (no categories) */}
        ...
        {showCart && !success && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-8 max-w-md w-full relative max-h-[80vh] flex flex-col">
              ...
              <button
                className="w-full px-4 py-2 rounded-lg bg-red-500 dark:bg-red-700 text-white font-semibold shadow hover:bg-red-600 dark:hover:bg-red-800 transition-colors mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={() => {
                  setSelectedItems({});
        ...
        <div className="text-center mt-6">
          <button
            onClick={() => navigate(`/order-forms?store=${store}`)}
